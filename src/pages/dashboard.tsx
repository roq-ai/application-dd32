...
import { getOfficers } from 'apiSdk/officers';
...
function DashboardPage() {
  ...
  const officerStat = useSWR<PaginatedInterface<OfficerInterface>>(
    () => `/officers`,
    () => getOfficers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Officers',
      result: officerStat?.data?.totalCount,
      isLoading: officerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
